...
// Babel can't find babelrc if we try to compile stuff in caches/avalheim-showdown-server/ fsr
...
if (process.argv[2] === 'full') {
	...
	compiler.compileToDir(
		['caches/avalheim-showdown-server/server/chat-formatter.ts', 'caches/avalheim-showdown-server/sim/teams.ts'],
...
try {
	...
	compiledFiles += compiler.compileToFile(
		[
			...
			'caches/avalheim-showdown-server/chat-formatter.ts',
...
try {
	...
	indexContents = indexContents
		.replace(/<!-- config\/head-custom\.html -->/g, '' + fs.readFileSync('config/head-custom.html'));
	preactIndexContents = preactIndexContents
		.replace(/<!-- config\/head-custom\.html -->/g, '' + fs.readFileSync('config/head-custom.html'));
	indexContents2 = indexContentsOld
		.replace(/<!-- config\/head-custom\.html -->/g, '' + fs.readFileSync('config/head-custom-test.html'));
} catch {}
