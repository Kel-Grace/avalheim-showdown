...
export class BattleTooltips {
	...
	calculateModifiedStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon, statStagesOnly?: boolean) {
		...
		if (weather) {
			...
			if (this.pokemonHasType(pokemon, 'Ice') && (weather === 'snowscape' || weather === 'eternalwinds')) {
				stats.def = Math.floor(stats.def * 1.5);
			}
			if (ability === 'sandrush' && weather === 'sandstorm') {
				speedModifiers.push(2);
			}
			if (ability === 'slushrush' && (weather === 'hail' || weather === 'snowscape' || weather === 'eternalwinds')) {
				speedModifiers.push(2);
			}
	...
	getMoveBasePower(move: Dex.Move, moveType: Dex.TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (
			move.id === 'watershuriken' && pokemon.getSpeciesForme() === 'Greninja-Bond' && pokemon.ability === 'Battle Bond'
		) {
